...
const char* ssid = "ID Tuananh";
const char* pass = "Tuananh151177";
...
void loop() {
    ...
    if (isEnrolling) {
        getFingerprintEnroll(client);
    } else if (isAttending) {
        getFingerprintAttend(client);
    }
}
...
void handleClient(WiFiClient &client) {   
    ...
    // Handle request
    if (request.indexOf("StartEnroll") >= 0 && !isEnrolling) {
        startEnroll();
    } else if (request.indexOf("StartAttend") >= 0 && !isAttending) {
        startAttend();
    }    
}


void getFingerprintEnroll(WiFiClient &client) {
    ...
    switch (state) {
        ...
        case READING_TEMPLATE:
            ...
            delay(1000);       
            for (int i = 0; i < 512; ++i) {
                if (fingerTemplate[i] < 16) client.print("0");
                client.print(fingerTemplate[i], HEX);
            }       
            client.print("\n");  
            isEnrolling = false;            
            break;
...
void getFingerprintAttend(WiFiClient &client) {
    static unsigned long lastActionTime = 0;
    const unsigned long interval = 2000; // 2 seconds delay
    const unsigned long removalDelay = 1000; // Reduced to 1 second
    const unsigned long enrollmentDelay = 200; // Reduced to 0.2 second
